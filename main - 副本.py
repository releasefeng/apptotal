...
<OneScreen>
    GridLayout:
	    ...
	    Button:
            text:"固件升级"
            font_name:'DroidSansFallback'
            on_press:root.manager.current="ota"
...
class SixScreen(Screen):
    def __init__(self, **kwargs):
        super(SixScreen, self).__init__(**kwargs)
    ...
    def getbid(self,entry):
        ...
        if entry == 0:
            ...
            try:
                temp=int(self.ids['IRbid'].text)
                if temp >=1 and temp <=10:
                    if temp==10:
                        bid='5a'
                    else:
                        bid='5'+str(self.ids['IRbid'].text).rstrip(' ').lstrip(' ')
                    Entry=1
                else:
                    self.ids['info'].text='BID不在1-10范围内'
            except:
                self.ids['info'].text='输入格式有误'
        elif entry == 2:
            bid='26'
            Entry=1
        elif entry == 3:
    ...
    def bootstart(self):
        self.ids['info'].text='重启进入boot'
        udpclient=socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        udpclient.settimeout(1)
        data='59'+'ee'+'51'+'01'+'00'+'00'
        CRCHi,CRCLo=self.calc(data.decode('hex'))
        tock=data+CRCLo+CRCHi+'47'
        udpclient.sendto(tock.decode('hex'),('192.168.80.201',6650))
        while True:
            print(tock)
            udpclient.sendto(tock.decode('hex'),('192.168.80.201',6650))
            try:
                temp=udpclient.recv(1024)
                result=temp.encode('hex')
                print(result)
                if result[2:4]=='ee':
                    if result[4:6]=='d1':
                        if result[8:10]=='00':
                            self.ids['info'].text='已进入boot模式'
                            break
                        else:
                            pass
                    else:
